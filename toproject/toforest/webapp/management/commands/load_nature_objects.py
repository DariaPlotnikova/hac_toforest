...
def load_parks():
    ...
    for obj in iter(park_objects):
        ...
        if created:
            count += 1
    print u'SUCCESS: Park objects load. Count %d' % count
    return count
...
def load_parks2():
    ...
    for obj in iter(park_objects):
        ...
        coords = coords.json().get('response').get('GeoObjectCollection').get('featureMember')[0].get('GeoObject').get(
            'Point').get('pos')
        time.sleep(0.1)
        ...
        if created:
            count += 1
    print u'SUCCESS: Park #2 objects load. Count %d' % count
    return count
